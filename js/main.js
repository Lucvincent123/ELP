...
function textFile(filePath) {
    return new Promise((resolve, reject) => fs.readFile(filePath, "utf-8", (error, text) => {
...
async function main(params) {
    ...
    for (let i = 0; i < game.getNumberOfPlayers(); i++) {
        ...
        newPlayer.setName((await input(`Player number ${i+1} name: `)).trim()) // input player's name
    ...
    while (! game.isNoMoreCardsLeft()) {
        ...
        let wordIndex = Number((await input("")).trim()) - 1
        ...
        for (let i = 0; i < game.numberOfPlayers - 1; i++) {
            ...
            game.getPlayerActive().easel = (await input("")).trim()
        }
        game.nextPlayer()
        game.showTurn()
